...
.then(data => {
    ...
    let movie = movies.find(movie => movie.id == id.get('id'));
    let text = document.getElementById('contenidoTextual');

    text.innerHTML = `<img class = "w-[70%] lg:w-[49%]" src="https://moviestack.onrender.com/static/${movie.image}" alt="${movie.title}">
